import asyncio
import edge_tts
# import playsound  # Not needed on macOS when using afplay
import simpleaudio as sa  # kept if you later switch to WAV playback
...
async def speak(text):
    ...
    communicate = edge_tts.Communicate(
        text,
        voice="ja-JP-NanamiNeural",  # try "en-US-JennyNeural" etc. if desired
        rate="-15%",
        pitch="+40Hz"
    )
    await communicate.save(filename)

    # macOS-native playback (blocks until finished)
    try:
...
if __name__ == "__main__":
    main()
async def speak(text, casual=False):
    """Make assistant speak with more natural variation."""
    print("AI:", text)
    filename = f"output_{int(time.time() * 800)}.mp3"
    communicate = edge_tts.Communicate(
        text,
        voice="en-US-JennyNeural",   # More natural English voice
        rate="-10%",
        pitch="+20Hz"
    )
    await communicate.save(filename)
...
async def handle_command(self, command):
    """Handle natural conversation flow and actions."""
    if command == "network error":
        self.add_text("[System] Network error")
        await speak("Hmm, I'm having trouble reaching the network.")
        return
    ...
    if "exit" in command or "quit" in command:
        self.add_text("[System] Exiting...")
        await speak("Alright, shutting down. Take care!")
        self.root.quit()
        return
    ...
    responses = [
        ...
        "Sorry, that went over my head. Want to try again?",
        "I didn’t catch that properly. Can you say it differently?",
    ]
